...
import { MessageCircle, X, Send, Rocket } from 'lucide-react';
...
const AIChat: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: 'Welcome to Carolina Grind! 🚀 Need help getting featured or finding talent?' }
  ]);
  ...
  return (
    <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-50 flex flex-col items-end pointer-events-auto">
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="mb-4 w-[90vw] md:w-96 bg-black/90 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden shadow-2xl shadow-[#fbbf24]/10"
          >
            {/* Header */}
            <div className="bg-gradient-to-r from-yellow-700/50 to-orange-900/50 p-4 flex justify-between items-center border-b border-white/10">
              <div className="flex items-center gap-2">
                <Rocket className="w-5 h-5 text-[#fbbf24]" />
                <h3 className="font-heading font-bold text-white tracking-wider">GRIND ASSISTANT</h3>
              </div>
                ...
                >
                  <div
                    className={`max-w-[85%] p-3 rounded-lg text-sm ${
                      msg.role === 'user'
                        ? 'bg-[#fbbf24] text-black font-medium rounded-tr-none'
            ...
            <div className="p-3 border-t border-white/10 bg-black/40">
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="bg-[#fbbf24] p-2 rounded-lg hover:bg-white transition-colors disabled:opacity-50 group"
                  data-hover="true"
                >
      ...
      <motion.button
        ...
        className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-[#fbbf24] flex items-center justify-center shadow-lg shadow-[#fbbf24]/20 border border-white/20 z-50 group"
        data-hover="true"
      >
        {isOpen ? (
